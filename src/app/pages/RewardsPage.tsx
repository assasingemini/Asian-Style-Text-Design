...
const tiers = [
  {
    ...
    range: '0 – 999',
    color: '#CD7F32',
    perks: ['1 điểm mỗi 1.000 VNĐ', 'Quà tặng sinh nhật', 'Truy cập sớm các đợt giảm giá'],
  },
  {
    name: 'Silver',
    range: '1.000 – 4,999',
    color: '#C0C0C0',
    perks: ['1.5 điểm mỗi 1.000 VNĐ', 'Miễn phí vận chuyển trọn đời', 'Sản phẩm độc quyền thành viên', 'Quà tặng sinh nhật x2'],
  },
  {
    name: 'Gold',
    range: '5,000 – 9,999',
    color: '#FFD700',
    perks: ['2 điểm mỗi 1.000 VNĐ', 'Chăm sóc ưu tiên', 'Lời mời sự kiện riêng tư', 'Quà tặng kèm đơn hàng', 'Quà tặng sinh nhật x3'],
  },
  {
    name: 'Platinum',
    range: '10,000+',
    color: '#E5E4E2',
    perks: ['3 điểm mỗi 1.000 VNĐ', 'Tư vấn phong cách riêng', 'Quyền mua sớm bộ sưu tập mới', 'Quà tặng độc quyền hàng tháng', 'Sự kiện VIP'],
  },
];

const rewards = [
  { id: 'r1', name: 'Giảm giá 50.000 VNĐ', points: 500, type: 'voucher', value: 50000 },
  { id: 'r2', name: 'Giảm giá 100.000 VNĐ', points: 900, type: 'voucher', value: 100000 },
  { id: 'r3', name: 'Mã miễn phí vận chuyển', points: 300, type: 'shipping', value: 30000 },
  { id: 'r4', name: 'Giảm giá 200.000 VNĐ', points: 1700, type: 'voucher', value: 200000 },
  { id: 'r5', name: 'Hộp quà bí mật', points: 2000, type: 'gift', value: 0 },
  { id: 'r6', name: 'Thẻ truy cập sớm', points: 1000, type: 'access', value: 0 },
];
...
export default function RewardsPage() {
  const { user, redeemPoints, showNotification } = useApp();
        ...
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex">
          {([
            ...
            <button
              ...
              className={`px-8 py-5 text-xs tracking-[0.25em] uppercase border-b-2 transition-all ${
          ...
          <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-black/10">
                {tiers.map(tier => {
                  ...
                  return (
                    ...
                    >
                      {isCurrent && (
                        <span className="text-[9px] tracking-[0.2em] uppercase bg-black text-white px-2 py-1 mb-4 inline-block">
                          Hạng của bạn
                        </span>
                      )}
                      ...
                      <ul className="space-y-2.5">
                        {tier.perks.map(perk => (
                          <li key={perk} className="flex items-start gap-2 text-xs text-black/60 tracking-wide">
                            <Check size={12} className="shrink-0 mt-0.5 text-black" />
                            {perk}
                          </li>
                        ))}
                      </ul>
            ...
            <div className="bg-[#F8F6F2] p-8 md:p-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                  { icon: '🛍️', action: 'Mua sắm', pts: '1 điểm mỗi 1.000 VNĐ' },
                  ...
                  <div key={item.action} className="flex items-center gap-5">
                    ...
                    <div>
                      <p className="text-sm tracking-wide mb-1">{item.action}</p>
                      <p className="text-xs text-black/40 tracking-wide">{item.pts}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                {user && <p className="text-black/40 text-sm mt-1">Bạn đang có <span className="text-black">{user.points.toLocaleString()}</span> điểm</p>}
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {rewards.map((reward, i) => {
                const canRedeem = user && user.points >= reward.points;
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <span className="text-[10px] tracking-[0.25em] uppercase border border-black/20 px-2 py-1">
                        {reward.points.toLocaleString()} điểm
                      </span>
                    </div>
                    <h3 className="font-['Cormorant_Garamond'] text-xl mb-1">{reward.name}</h3>
                    {reward.value > 0 && (
                      <p className="text-black/40 text-xs tracking-wide mb-5">Trị giá {formatPrice(reward.value)}</p>
                    )}
                    {!reward.value && (
                      <p className="text-black/40 text-xs tracking-wide mb-5">Phần thưởng độc quyền</p>
                    )}
                    <button
                      onClick={() => handleRedeem(reward)}
                      disabled={!canRedeem || !!isLoading}
                      className={`w-full text-xs tracking-[0.2em] uppercase py-3 transition-all flex items-center justify-center gap-2 ${
                        canRedeem
                    ...
                    >
                      ...
                      ) : canRedeem ? 'Đổi ngay' : 'Cần thêm điểm'}
                    </button>
                  </motion.div>
                );
              })}
                ...
                >
                  <div className="flex items-center gap-5">
                    <div className={`w-8 h-8 flex items-center justify-center rounded-full ${
                      item.type === 'earned' ? 'bg-black/5' : 'bg-red-50'
                    }`}>
                      {item.type === 'earned' ? <Star size={14} className="text-black/40" /> : <Gift size={14} className="text-red-400" />}
                    </div>
                    <div>
                      <p className="text-sm tracking-wide">{item.action}</p>
                      <p className="text-xs text-black/40 tracking-wide mt-0.5">{item.date}</p>
                    </div>
                  </div>
