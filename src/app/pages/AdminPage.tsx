...
import {
  LayoutDashboard, Package, ShoppingCart, Users, FileText, Zap, Award,
  Plus, Pencil, Trash2, Search, TrendingUp, DollarSign, Eye, ChevronDown, X, Menu
} from 'lucide-react';
...
type AdminTab = 'dashboard' | 'products' | 'orders' | 'users' | 'blog' | 'flash' | 'rewards';
        ...
        <div className="p-6 md:p-8">
                ...
                {stats.map((stat, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="bg-white p-6 border border-black/10"
                    >
                      <div className="flex items-start justify-between mb-4">
                        <Icon size={16} className="text-black/40" />
                        <span className={`text-[10px] tracking-wider ${stat.positive ? 'text-green-600' : 'text-red-500'}`}>
                          {stat.change}
                        </span>
                      </div>
                ...
                <div className="lg:col-span-2 bg-white p-6 border border-black/10">
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <AreaChart data={revenueData}>
                      ...
                      <Tooltip
                        contentStyle={{ border: '1px solid #e0e0e0', borderRadius: 0, fontSize: 11 }}
                ...
                <div className="bg-white p-6 border border-black/10">
                  ...
                  <ResponsiveContainer width="100%" height={140}>
                    <PieChart>
                      ...
                      <Tooltip contentStyle={{ fontSize: 11, border: '1px solid #e0e0e0', borderRadius: 0 }} />
                    </PieChart>
                  </ResponsiveContainer>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-black/10">
                        {['Mã đơn hàng', 'Khách hàng', 'Sản phẩm', 'Tổng cộng', 'Ngày', 'Trạng thái'].map(h => (
                          <th key={h} className="text-left pb-3 text-[9px] tracking-[0.25em] uppercase text-black/40 font-normal pr-6">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    ...
                    <thead>
                      <tr className="border-b border-black/10 bg-[#F8F8F6]">
                        {['Sản phẩm', 'Danh mục', 'Giá', 'Tồn kho', 'Trạng thái', 'Thao tác'].map(h => (
                          <th key={h} className="text-left px-6 py-4 text-[9px] tracking-[0.25em] uppercase text-black/40 font-normal">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={product.id} className="border-b border-black/5 hover:bg-black/2 transition-colors">
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-12 bg-[#F5F5F3] overflow-hidden shrink-0">
                                <img src={product.images[0]} alt="" className="w-full h-full object-cover" />
                              </div>
                              <div>
                                <p className="tracking-wide font-medium">{product.name}</p>
                                <p className="text-black/30 text-[10px] mt-0.5">{product.id}</p>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-4">
                            <span className={`text-[9px] tracking-wider px-2 py-1 ${product.stock <= 5 ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
                              {product.stock} chiếc
                            </span>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex gap-1 flex-wrap">
                              {product.isNew && <span className="text-[9px] tracking-wider bg-black text-white px-1.5 py-0.5">Mới</span>}
                              {product.isBestseller && <span className="text-[9px] tracking-wider bg-gray-100 px-1.5 py-0.5">Bán chạy</span>}
                              {product.isFlashSale && <span className="text-[9px] tracking-wider bg-red-100 text-red-700 px-1.5 py-0.5">Sale</span>}
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex gap-2">
                              <button
                                ...
                                className="w-7 h-7 border border-black/15 flex items-center justify-center hover:border-black transition-colors"
                              >
                                <Pencil size={11} />
                              </button>
                              <button className="w-7 h-7 border border-red-200 flex items-center justify-center hover:bg-red-50 transition-colors text-red-400">
                                <Trash2 size={11} />
                              </button>
                            </div>
                          </td>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="p-6 grid grid-cols-2 gap-5">
                        {[
                          { label: 'Tên sản phẩm', placeholder: 'Áo khoác Void Oversized', full: true },
                          { label: 'Giá (VNĐ)', placeholder: '289000' },
                          { label: 'Giá gốc', placeholder: '389000' },
                          { label: 'Tồn kho', placeholder: '20' },
                        ].map(field => (
                        ...
                        <div className="col-span-2">
                          <label className="block text-[10px] tracking-[0.25em] uppercase text-black/50 mb-2">Danh mục</label>
                          <select className="w-full border border-black/20 px-4 py-3 text-sm outline-none focus:border-black bg-white">
                            {['Áo khoác', 'Áo', 'Quần & Váy', 'Đầm', 'Phụ kiện'].map(cat => (
                              <option key={cat} value={cat}>{cat}</option>
                            ))}
                          </select>
                        </div>
                        <div className="col-span-2">
                          <label className="block text-[10px] tracking-[0.25em] uppercase text-black/50 mb-2">Mô tả</label>
                          <textarea
                            rows={3}
                            className="w-full border border-black/20 px-4 py-3 text-sm outline-none focus:border-black transition-colors resize-none tracking-wide"
                      ...
                      <div className="p-6 border-t border-black/10 flex gap-3 justify-end">
                        <button
                          onClick={() => { setShowAddProduct(false); setEditingProduct(null); }}
                          className="px-6 py-3 border border-black/20 text-xs tracking-[0.2em] uppercase hover:border-black transition-colors"
                        >
                          Hủy
                        </button>
                        <button
                          onClick={() => { setShowAddProduct(false); setEditingProduct(null); }}
                          className="px-6 py-3 bg-black text-white text-xs tracking-[0.2em] uppercase hover:bg-black/90 transition-colors"
                        >
          ...
          {activeTab === 'orders' && (
            <div className="bg-white border border-black/10">
              <div className="p-6 border-b border-black/10">
                <h2 className="font-['Cormorant_Garamond'] text-2xl">Quản lý đơn hàng</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-black/10 bg-[#F8F8F6]">
                      {['Mã đơn hàng', 'Khách hàng', 'Sản phẩm', 'Tổng cộng', 'Ngày', 'Trạng thái', 'Thao tác'].map(h => (
                        <th key={h} className="text-left px-6 py-4 text-[9px] tracking-[0.25em] uppercase text-black/40 font-normal">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {mockOrders.map(order => (
                      <tr key={order.id} className="border-b border-black/5 hover:bg-black/2 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            <button className="text-[9px] tracking-wider border border-black/20 px-2.5 py-1 hover:border-black transition-colors">Xem</button>
                            <button className="text-[9px] tracking-wider border border-black/20 px-2.5 py-1 hover:border-black transition-colors">Cập nhật</button>
                          </div>
                        </td>
                      </tr>
                    ))}
            ...
            <div className="bg-white border border-black/10">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-black/10 bg-[#F8F8F6]">
                      {['Người dùng', 'Email', 'Hạng', 'Điểm', 'Đơn hàng', 'Tham gia', 'Thao tác'].map(h => (
                        <th key={h} className="text-left px-6 py-4 text-[9px] tracking-[0.25em] uppercase text-black/40 font-normal">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {mockUsers.map(u => (
                      <tr key={u.id} className="border-b border-black/5 hover:bg-black/2 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-[9px] tracking-wider border border-black/20 px-2 py-0.5">{u.tier}</span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            <button className="text-[9px] tracking-wider border border-black/20 px-2.5 py-1 hover:border-black transition-colors">Xem</button>
                            <button className="text-[9px] tracking-wider border border-red-200 px-2.5 py-1 hover:bg-red-50 text-red-500 transition-colors">Chặn</button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          {activeTab === 'blog' && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <button className="flex items-center gap-2 bg-black text-white text-xs tracking-[0.2em] uppercase px-5 py-3 hover:bg-black/90 transition-colors">
              ...
              <div className="space-y-4">
                {blogPosts.map(post => (
                  <div key={post.id} className="bg-white border border-black/10 p-5 flex items-center gap-5">
                    <div className="w-16 h-16 overflow-hidden bg-[#F5F5F3] shrink-0">
                    ...
                    <div className="flex gap-2 shrink-0">
                      <button className="w-7 h-7 border border-black/15 flex items-center justify-center hover:border-black transition-colors">
                        <Pencil size={11} />
                      </button>
                      <button className="w-7 h-7 border border-red-200 flex items-center justify-center hover:bg-red-50 text-red-400 transition-colors">
                        <Trash2 size={11} />
                      </button>
                    </div>
                  </div>
          ...
          {activeTab === 'flash' && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <button className="flex items-center gap-2 bg-black text-white text-xs tracking-[0.2em] uppercase px-5 py-3">
                  ...
                  <div key={campaign.name} className="bg-white border border-black/10 p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        <p className="text-xs text-black/40 mt-1">{campaign.products} sản phẩm · giảm {campaign.discount}%</p>
                      </div>
                      <span className={`text-[9px] tracking-[0.2em] uppercase px-2.5 py-1 ${
                        campaign.status === 'Hoạt động' ? 'bg-green-100 text-green-800' :
                        campaign.status === 'Đã lên lịch' ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-[9px] tracking-wider text-black/40 uppercase mb-1">Còn lại</p>
                        <p className="text-sm font-medium">{campaign.ends}</p>
                      </div>
                      <div>
                        <p className="text-[9px] tracking-wider text-black/40 uppercase mb-1">Doanh thu</p>
                        <p className="text-sm font-['Cormorant_Garamond'] text-base">{campaign.revenue}</p>
                      </div>
                    </div>
                    <div className="flex gap-2 mt-5">
                      ...
                      {campaign.status === 'Hoạt động' && (
                        <button className="text-[9px] tracking-wider border border-red-200 text-red-500 px-3 py-1.5 hover:bg-red-50 transition-colors">Kết thúc</button>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {activeTab === 'rewards' && (
            <div>
              ...
              <div className="bg-white border border-black/10 p-6">
                <h3 className="font-['Cormorant_Garamond'] text-xl mb-5">Phân bổ hạng thành viên</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={[
                    { tier: 'Bronze', count: 987 },
                    { tier: 'Silver', count: 524 },
                    { tier: 'Gold', count: 289 },
                    { tier: 'Platinum', count: 47 },
                  ]}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="tier" tick={{ fontSize: 10, fill: '#888' }} axisLine={false} tickLine={false} />
